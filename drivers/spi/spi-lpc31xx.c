...
#define SPI_FIFO_DATA_REG_OFFSET 0x0C
...
static int configure_dma(struct lpc31xx_spi *espi)
{
	struct dma_slave_config rx_conf = {
		.src_addr = espi->phybase + SPI_FIFO_DATA_REG_OFFSET,
		.direction = DMA_DEV_TO_MEM,
		.device_fc = false,
	};
	struct dma_slave_config tx_conf = {
		.dst_addr = espi->phybase + SPI_FIFO_DATA_REG_OFFSET,
	...
	case SSP_RX_32_OR_MORE_ELEM:
		...
		break;
	default:
		rx_conf.src_maxburst = SPI_FIFO_DEPTH  >> 1;
		break;
	}
	...
	case SSP_TX_32_OR_MORE_EMPTY_LOC:
		...
		break;
	default:
		tx_conf.dst_maxburst = SPI_FIFO_DEPTH  >> 1;
		break;
	}
				...
				struct spi_transfer *transfer)
{
	...
	dev_dbg(&espi->pdev->dev, "Transfers %d size %d", espi->cur_transfer->len, espi->cur_chip->n_bytes);
...
static void pump_transfers(unsigned long data)
{
	...
	if (espi->cur_chip->enable_dma && (espi->cur_transfer->len > SPI_FIFO_DEPTH)) {
...
lpc31xx_probe(struct platform_device *pdev)
{
	...
	/* Get DMA channels */
	espi->enable_dma = (lpc31xx_dma_probe(espi) == 0);
